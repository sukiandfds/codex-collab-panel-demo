...
- 员工是长期独立主体；员工直属单聊和员工参与每个项目群聊的长期会话始终属于员工自己的项目。
- 工作项目拥有项目文件、项目群聊和完整公共聊天记录，但不能拥有员工背后的 Runtime Thread。
- 员工可以访问和操作目标工作项目；所有者项目、目标项目和文件路径是三个不同字段，不能根据 `cwd` 或当前页面猜测。
- 一个员工参与多个项目时，各项目群聊会话相互隔离并统一保留在员工项目下。
- 项目群聊归档、删除或员工退出目前不联动删除员工会话。
...
| 普通项目过滤员工 Thread | `employee-project-directory` 过滤员工主 Thread 和房间 Agent Thread | 产品规则已明确，当前实现仍需修复并验证不再混入 |
...
| 员工会话归属固定 | 员工直属单聊和每个项目群聊长期会话始终绑定员工项目；目标项目只提供群聊和文件工作范围 |
