...
schema_version: 1
last_updated: 2026-08-01 23:51 +08:00
...
| `PROC-022` | `active` | 实时状态/事件顺序 | 跨设备会话在终态恢复、流式回答交接和新 Turn 切换时出现旧状态残留、已显示文字短暂消失或上一 Turn 过程串入当前 Turn | 统一使用 `runId/turnId/eventSeq` 约束事件单调性；终态事件和持久化快照优先级高于本地运行文案；流式缓冲只允许追加并在最终消息确认后一次性交接；恢复和新 Turn 都必须丢弃旧缓冲 | `FEAT-001-I24`、用户 2026-08-01 实测反馈 |
...
### PROC-022：实时会话终态与流式事件交接不具备统一顺序

- 发现时间：2026-08-01 23:51 +08:00
- 分类：普适
- 状态：active
- 关联功能：`FEAT-001-I24`、`FEAT-005`
- 用户实际现象：跨设备发送消息时，用户消息和处理中状态可以实时同步；但上次任务结束后重新打开页面，未刷新时仍显示“正在处理中”；流式最终回答接近结束时，已显示的一段文字会突然消失，数秒后才恢复；隔一段时间重新发送新消息时，页面会短暂显示上一次 Turn 的思考过程。
- 用户影响：用户无法判断页面展示的是当前任务、旧任务还是尚未补齐的结果；回答内容短暂消失会造成“丢回复”的体感，旧过程串入新 Turn 会降低对执行状态的信任。
- 已验证事实：实时事件快路径本身能够把跨设备消息和过程推到其他设备；问题集中发生在终态恢复、流式缓冲与新 Turn 切换的交接阶段。具体事件序列和服务端快照仍待抓取，不能把某一个 React 状态或 SSE 回调直接写成确定根因。
- 初步根因边界：当前缺少贯穿实时事件、恢复快照、本地流式草稿和最终持久化消息的统一 `runId/turnId/eventSeq` 单调约束；因此旧快照或旧 Turn 事件可能覆盖新显示，或者最终消息交接时暂时替换掉仍在展示的流式缓冲。
- 错误路径：把“收到实时内容”误当成“会话生命周期已经正确收口”，只验证消息能否即时出现，没有验证页面重开、最终事件、持久化快照和下一 Turn 的顺序关系。
- 正确路径：先为每个 Run/Turn 建立权威生命周期和单调事件序号；终态事件与持久化快照优先于本地运行文案；流式答案只允许追加，最终消息确认后再原子交接；页面恢复时先以权威快照清理旧运行态；新 Turn 建立独立缓冲并拒绝旧 Turn 事件。
- 防再犯触发器：只要出现“流式文字消失”“重新打开仍显示处理中”“新消息短暂显示旧过程”任一现象，必须保存同一 Turn 的事件序号、快照时间、终止事件和渲染缓冲，再开始修改；禁止先增加轮询、延迟或状态文案。
- 证据：用户 2026-08-01 实测反馈；`FEAT-001-I08`、`I15`、`I17` 的既有记录。
