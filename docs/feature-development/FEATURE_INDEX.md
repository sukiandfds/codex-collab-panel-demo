...
| --- | --- | --- | --- | --- | --- | --- |
| `FEAT-001` | 单人 Codex Web 对话与控制 | `implemented_pending_review` | `v0.9.0` | 2026-07-28 17:43 +08:00 | 已实现事件游标重连、断档识别和状态/会话快照补偿；自动测试通过，仍需原开发电脑真实长任务确认 1–2 分钟停更是否消失 | [`FEAT-001-single-codex-web.md`](./features/FEAT-001-single-codex-web.md) |
...
| --- | --- | --- | --- | --- |
| 1 | 单人 Codex Web 基础能力与持续过程同步 | `FEAT-001` 已实现，恢复修复待实机验收 | 网页已有真实对话、发送、追加、停止和过程展示；本轮新增事件重放、断档识别和权威快照校正，自动测试已通过，但尚未在原开发电脑复测用户遇到的 1–2 分钟停更 | Web 按 Codex 的真实执行事件持续显示分析、命令、文件修改、测试和最终回复；断线、漏事件或后台恢复后自动补齐，不要求用户重开页面。“修改文件”仍属于原有过程同步 |
...
| 6 | 手机/PWA、设备身份和稳定远程入口 | `FEAT-004` 已实现待体验；`FEAT-006` 进行中 | 手机、iPad 和电脑浏览器已有适配；新构建会提示刷新且不自动打断任务，顶部只描述网页同步状态。当前远程入口仍主要依赖 Quick Tunnel | 用户保存一个受保护的网址，在手机或其他电脑打开即可继续使用；页面准确区分“原电脑离线、网页服务离线、Codex 任务运行中”，并在代码更新后由用户决定何时刷新 |
...
| --- | --- | --- | --- |
| `P1` | `FEAT-001-I08` 持续过程同步 | 在原开发电脑验收已完成的事件重放、断档识别和快照补偿 | 使用 5 分钟以上真实任务、断网重连、后台切回和完成事件遗漏；文件修改、测试仍沿用 Codex 事件链路 |
...
| `P4` | `FEAT-009` 无 UAC 远程开发与应急恢复 | 在原电脑运行只读检查器和真实普通权限开发链路；再确定最小应急启动入口 | 检查器已经实现；不先做电脑重启恢复、双 Worker、自动回滚或大型 Supervisor |
