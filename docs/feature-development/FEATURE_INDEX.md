...
| `FEAT-015` | 自然语言生图与自动化工作台 | `planned` | `v0.2.0` | 2026-08-04 15:18 +08:00 | 已完成 HappyEvering 官方协议和临时 Skill 点检；下一步定义 Capability/Provider Schema 与 Mock 合同测试，再接入 Codex 自然语言工具 | [`FEAT-015-image-generation-and-automation-workbench.md`](./features/FEAT-015-image-generation-and-automation-workbench.md) |

`FEAT-010` 曾用于已暂停的 Orca 路线实验，本轮不复用该编号。

## 完整需求表（按实际使用链路合并）

本表覆盖当前正式功能 `FEAT-001` 至 `FEAT-009`、`FEAT-011` 至 `FEAT-015`，并保留已冻结的 `FEAT-010` 技术试验记录。相互影响的功能放在同一行；当前状态和目标体验严格分开。
...
| 10 | 自然语言生图与专门的生图自动化工作台 | `FEAT-015 v0.2.0` 已完成协议核查 | 当前尚未接入中转站；HappyEvering 合同、临时 Skill 问题和正式运行边界已经记录 | 普通对话可以自然触发生图；批量、模板、队列、历史和定时任务进入专门工作台，并与对话共用同一运行、媒体、交付物和审计链路 |
...
| `P4.5` | `FEAT-015` 自然语言生图与自动化工作台 | 定义 Capability/Provider Schema 和 Mock 合同测试，完成一个可追踪的对话生图闭环；工作台、批量和定时任务按后续里程碑增加 | Skill 只负责工作流，不让前端或单个脚本直接拥有密钥、供应商协议和任务状态 |
