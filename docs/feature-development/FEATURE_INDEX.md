...
schema_version: 1
last_updated: 2026-08-04 17:16 +08:00
---
...
| `FEAT-015` | 自然语言生图与自动化工作台 | `in_progress` | `v0.4.0` | 2026-08-04 18:39 +08:00 | 原生 Negus Image MCP 与网页最小闭环已完成；真实 2K 原生生图和 1K 网页生图通过，结果、状态及重启恢复可用，工作台与定时任务待后续开发 | [`FEAT-015-image-generation-and-automation-workbench.md`](./features/FEAT-015-image-generation-and-automation-workbench.md) |
...
| 10 | 自然语言生图与专门的生图自动化工作台 | `FEAT-015 v0.4.0` 开发中 | 原生 Codex MCP 与网页关键词生图均已接入；网页可显示状态、图片和大图，服务重启后会话与图片仍保留 | 批量、模板、队列、历史和定时任务进入专门工作台，并与对话共用同一运行、媒体、交付物和审计链路 |
...
| `P4.5` | `FEAT-015` 自然语言生图与自动化工作台 | 用户拉取后验收网页最小闭环；后续再建设 Artifact、批量模板、队列和定时任务 | 前端不拥有密钥和供应商协议；工作台继续复用现有 Provider、运行记录和媒体链路 |
