...
> 后续方向澄清：用户希望的是一次性形成能够替代旧 Demo 的新版本，而不是按模块逐个交付和长期修补旧底座。技术实现仍有两条未决路径：整体 Fork Orca 后删改，或在现有项目中一次性迁入全部必要能力。两者尚未完成依赖、删减难度、维护成本和最终体验对比，当前不能直接判定哪条正确。
...
本报告最初建议采用 **第 3 种和第 4 种的组合**，但该建议没有获得用户确认，不能视为已经确定的路线：
...
Runtime 技术试验已经证明“可以写适配层”，但没有证明产品路线成立。后续不再按上述清单直接开发，必须先完成两条整体迁移路径的客观比较。

### 最终判断

目前唯一已经确认的产品目标是：利用 Orca 已经稳定的基础，一次性交付能够替代旧 Demo 的完整版本，再加入本项目的群聊、共享上下文和 AI 小公司协作体验。

具体技术路线仍需在以下两项之间做出有证据的选择：

1. 以 Orca 整体代码库为新底座，删除无关功能并加入本项目能力；
2. 保留当前项目外壳，一次性迁入所有需要的 Orca 成熟能力，完成后整体切换。

单独完成 Runtime Adapter、逐个交付零散模块或继续给旧 Demo 打补丁，都不等于完成上述目标。当前报告不替用户拍板选择其中一条。
