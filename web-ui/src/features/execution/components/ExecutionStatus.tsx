...
export function ExecutionStatus({ connected, status, commentary }: { connected: boolean; status: ExecutionStatusValue; commentary: string }) {
  if (!connected) {
  ...
  return (
    <div className={styles.summary}>
      ...
      {status.active && commentary ? <span className={styles.commentary}>{commentary}</span> : null}
    </div>
  );
}
