...
export function AgentRoster({ agents, onOpenProfile }: {
  agents: GroupAgent[];
  onOpenProfile: (agent: GroupAgent) => void;
}) {
  return (
    <aside className={styles.agentPanel}>
      ...
      <div className={styles.agentList}>
        {agents.map((agent) => (
          <button className={styles.agentRow} type="button" key={agent.id} onClick={() => onOpenProfile(agent)}>
