...
当前阶段：单项目真实对话 Web Demo + 实验性真实群聊/多 Agent Demo；核心链路已跑通，群聊交互尚未验收

## 1. 最重要的维护边界

1. **当前只专注一个项目中的真实 Codex 对话及其 Web 控制闭环。** 2026-07-24 新增的群聊页面只是隔离的单项目技术原型，交互体验尚未通过用户验收；不要继续扩展跨项目聚合、完整多人权限或企业协作系统。
...
> 先把一个项目中的真实 Codex 对话稳定、快速、准确地放进 Codex 风格 Web UI，建立可靠的数据、渲染、性能和代码维护基础。
...
### 第一优先：明确 Desktop 与 Web 的同步边界

先验证 Desktop 重新打开 Thread 后是否稳定重载外部记录，再评估 daemon/proxy/IPC 或统一 Connector。不要编辑 JSONL，不伪造 Desktop 消息。

### 第二优先：限制双端并发并补足执行控制

统一同步完成前，同一 Thread 同一时刻只使用一个发送入口；后续按需增加 `turn/steer`、`turn/interrupt` 和审批状态，但不顺带改 UI。
...
只有单项目对话的真实性、渲染、性能、维护边界和安全机制稳定后，才进入多用户身份、项目权限、共享状态、群聊和人工确认的指令转发。
